async function saveData(event) {
    ...
    try {
        let res = await axios.post('http://localhost:5500/add-bill', BillObj);
        display(res.data);
    } catch (err) {
        console.log(err);
    }
...
async function display(BillObj) {
    ...
    deleteButton.onclick = async() => {
        try {
            let res = await axios.post('http://localhost:5500/delete-bill/', { id });
            console.log(res);
        } catch (err) {
            console.log(err);
        }
...
window.addEventListener('DOMContentLoaded', async() => {
    try {
        let res = await axios.get('http://localhost:5500/add-bill')
